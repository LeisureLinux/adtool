...
static char *system_config_file = AD_CONFIG_FILE;
char *config_file;

static char *uri = NULL;
static char *binddn = NULL;
static char *bindpw = NULL;
static char *search_base = NULL;
...
LDAP *ad_login() {
  ...
  if (user_config_path != NULL) {
    options_path_length =
        strlen(user_config_path) + strlen(user_config_filename) + 1;
    user_config_file = malloc(options_path_length);
    snprintf(user_config_file, options_path_length, "%s%s", user_config_path,
  ...
  if (!uri) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error: couldn't read active directory uri parameter from config "
             "file %s, ~/.adtool.cfg or command line",
             config_file);
  ...
  if (!binddn) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error: couldn't read active directory binddn parameter from "
             "config file %s, ~/.adtool.cfg or command line",
             config_file);
    ad_error_code = AD_MISSING_CONFIG_PARAMETER;
    return 0;
  }
  if (!bindpw) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error: couldn't read active directory bindpw (bind password) "
             "parameter from config file %s, ~/.adtool.cfg or command line",
             config_file);
    ad_error_code = AD_MISSING_CONFIG_PARAMETER;
    return 0;
  }
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error doing ldap_initialize on uri %s: %s", uri,
  ...
  if (result != LDAP_OPT_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ldap_set_option (protocol->v3): %s",
             ldap_err2string(result));
    ad_error_code = AD_SERVER_CONNECT_FAILURE;
    return 0;
  }
  ...
  if (result != LDAP_OPT_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ldap_set_option (referrals=0): %s",
             ldap_err2string(result));
    ad_error_code = AD_SERVER_CONNECT_FAILURE;
    return 0;
  }
  ...
  if (bindresult != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH, "Error in ldap_bind %s",
             ldap_err2string(bindresult));
    ad_error_code = AD_SERVER_CONNECT_FAILURE;
    ldap_perror(ds, "bind: ");
    return 0;
  }
...
char *ad_get_error() { return ad_error_msg; }

/* return the last error code generated */
int ad_get_error_num() { return ad_error_code; }
...
int ad_create_user(char *username, char *dn) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH, "Error in ldap_add %s",
             ldap_err2string(result));
...
int ad_object_delete(char *dn) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH, "Error in ldap_delete: %s",
             ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
...
int ad_setpass(char *dn, char *password) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ldap_modify for password: %s", ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
  }
...
char **ad_search(char *attribute, char *value) {
  ...
  if (!search_base) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error: couldn't read active directory searchbase parameter from "
             "config file %s, ~/.adtool.cfg or command line",
             config_file);
    ad_error_code = AD_MISSING_CONFIG_PARAMETER;
    return (char **)-1;
  ...
  result = ldap_search_s(ds, search_base, LDAP_SCOPE_SUBTREE, filter, attrs, 1,
                         &res);
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
  ...
  if (num_results == 0) {
    ldap_msgfree(res);
    snprintf(ad_error_msg, MAX_ERR_LENGTH, "%s not found", value);
...
int ad_mod_add(char *dn, char *attribute, char *value) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ad_mod_add, ldap_mod_s: %s\n", ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
  }
...
int ad_mod_add_binary(char *dn, char *attribute, char *data, int data_length) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ad_mod_add_binary, ldap_mod_s: %s\n",
             ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
...
int ad_mod_replace(char *dn, char *attribute, char *value) {
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ad_mod_replace, ldap_mod_s: %s\n",
             ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
...
int ad_mod_replace_binary(char *dn, char *attribute, char *data,
                          int data_length) {
  LDAP *ds;
  LDAPMod *attrs[2];
  LDAPMod attr;
  ...
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ad_mod_replace_binary, ldap_mod_s: %s\n",
             ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
  } else {
    ad_error_code = AD_SUCCESS;
...
char **ad_get_attribute(char *dn, char *attribute) {
  ...
  result =
      ldap_search_s(ds, dn, LDAP_SCOPE_BASE, "(objectclass=*)", attrs, 0, &res);
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
  ...
  if (num_entries == 0) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "No entries found in ad_get_attribute for user %s.", dn);
    ldap_msgfree(res);
    ad_error_code = AD_OBJECT_NOT_FOUND;
    return NULL;
...
int ad_move_user(char *current_dn, char *new_container) {
  ...
  result = ldap_rename_s(ds, current_dn, exdn[0], new_container, 1, NULL, NULL);
  ldap_memfree(exdn);
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
...
int ad_group_subtree_remove_user(char *container_dn, char *user_dn) {
  ...
  filter = malloc(filter_length);
  snprintf(filter, filter_length, "(&(objectclass=group)(member=%s))", user_dn);

  result = ldap_search_s(ds, container_dn, LDAP_SCOPE_SUBTREE, filter, attrs, 0,
                         &res);
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
...
char **ad_list(char *dn) {
  ...
  result = ldap_search_s(ds, dn, LDAP_SCOPE_ONELEVEL, "(objectclass=*)", attrs,
                         0, &res);
  if (result != LDAP_SUCCESS) {
    snprintf(ad_error_msg, MAX_ERR_LENGTH,
             "Error in ldap_search_s for ad_list: %s", ldap_err2string(result));
    ad_error_code = AD_LDAP_OPERATION_FAILURE;
    return NULL;
  }
  num_entries = ldap_count_entries(ds, res);
