...
void usage() {
  printf(
      ...
      "\n"
      "These options may alternatively be read from %s or ~/.adtool.cfg.  "
      ...
      "userlock           <username>                      disable a user "
      "account\n"
      "userunlock         <username>                      enable a user "
      "account\n"
      "setpass            <user> [password]               set user's password\n"
      "usermove           <user> <new container>          move user to another "
      "container\n"
      "userrename         <old username> <new username>   rename user\n"
      "\n"
      "computercreate     <computer name> <container>     create a computer "
      "account\n"
      "\n"
      "groupcreate        <group name> <container>        create a new group\n"
      "groupdelete        <group name>                    delete a group\n"
      "groupadduser       <group> <user>                  add a user to a "
      "group\n"
      "groupremoveuser    <group> <user>                  remove a user from a "
      "group\n"
      "groupsubtreeremove <container> <user>              remove a user from "
      "all groups below a given ou\n"
      "\n"
      "oucreate           <organizational unit name> <container>\n"
      "                                                   create a new "
      "organizational unit\n"
      "oudelete           <organizational unit name>      delete an "
      "organizational unit\n"
      "\n"
      "attributeget       <object> <attribute>            display attribute "
      "values\n"
      "attributeadd       <object> <attribute> <value>    add an attribute\n"
      "attributeaddbinary <object> <attribute> <filename> add an attribute "
      "from a file\n"
      "attributereplace   <object> <attribute> <value>    replace an "
      "attribute\n"
      "attributedelete    <object> <attribute> [value]    delete an attribute "
      "or attribute instance\n"
      "\n"
...
void setpass(char **argv) {
  ...
  if (result != AD_SUCCESS) {
    fprintf(stderr, "error: %s\n", ad_get_error());
    fprintf(stderr, "Ensure openldap is built with ssl support, and that you "
                    "are using a secure connection to your active directory "
                    "server (ldaps:// rather than plain ldap://).\n");
    exit(1);
  }
}
